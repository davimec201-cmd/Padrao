...
Há dois caminhos para chegar nesses rótulos, e as salvaguardas são as mesmas nos
dois porque moram em `aplicar()`:

* **na sessão** (o caminho normal) — `pedido()` monta o que precisa ser decidido,
  o modelo que está conduzindo a conversa responde, e `aplicar()` recebe a
  resposta. Sem chave de API, sem rede, sem custo separado.
* **por API** (para automação sem ninguém na sala) — `classificar()` faz a mesma
  coisa chamando a API e passando a resposta para `aplicar()`.
...
class Resultado:
    aplicadas: int = 0
    confirmadas: int = 0  # trechos em que o rótulo escolhido já era o atual
    rodou: bool = False
...
def _catalogo_em_texto() -> str:
    return "\n".join(f"- {nome}: {CATALOGO[nome].descricao}" for nome in CLASSIFICAVEIS)


def pedido(blocos: list[Bloco]) -> dict:
    """O que ainda precisa de rótulo, pronto para ser decidido na sessão.

    Devolve as instruções, o catálogo e os trechos candidatos. Quem responde —
    modelo na sessão ou API — devolve a lista de `classificacoes` para
    `aplicar()`. Lista vazia de trechos significa que o markdown já se explicou
    sozinho e não há nada a decidir.
    """
    indices = candidatos(blocos)
    return {
        "instrucoes": INSTRUCOES.format(catalogo=_catalogo_em_texto()),
        "tipos_permitidos": list(CLASSIFICAVEIS),
        "esquema_da_resposta": ESQUEMA,
        "trechos": [
            {"indice": indice, "resumo": _resumo(blocos[indice])} for indice in indices
        ],
    }


def aplicar(blocos: list[Bloco], classificacoes: list[dict]) -> Resultado:
    """Aplica rótulos aos blocos genéricos, com as salvaguardas.

    É aqui que mora a disciplina do sistema, e é por isso que os dois caminhos
    (sessão e API) passam por esta função em vez de cada um aplicar do seu jeito:
    rótulo fora do catálogo é descartado, bloco que veio de diretiva não se toca,
    e promoção só vale com confiança alta.
    """
    resultado = Resultado(rodou=True)
    validos = set(candidatos(blocos))

    for entrada in classificacoes or []:
        ...
        if tipo == blocos[indice].tipo:
            # confirmar o genérico também é uma decisão: o trecho deixa de ser
            # candidato e sai do relatório. Sem isso, um material inteiro bem
            # classificado continuaria acusando trechos "por conferir".
            if confianca == "alta":
                blocos[indice].procedencia = "classificador"
                resultado.confirmadas += 1
            else:
                blocos[indice].observacao = (
                    f"mantido como {CATALOGO[tipo].rotulo} com confiança baixa — {motivo}"
                )
        ...
        if confianca != "alta":
            blocos[indice].observacao = (
                f"considerado '{CATALOGO[tipo].rotulo}' com confiança baixa; "
...
def classificar(blocos: list[Bloco], cliente=None) -> Resultado:
    ...
    return aplicar(blocos, dados.get("classificacoes", []))
