...
def classificacao(resultado_classificador, blocos) -> Achado:
    """O que o catálogo não resolveu sozinho.

    Um trecho sem diretiva que ninguém rotulou não está errado — ele vira seção
    conceitual, que é o fallback correto. Mas ficar calado sobre isso seria
    esconder a única decisão que o sistema delega a um modelo, então cada caso
    aparece aqui com o número do bloco e o título, para poder ser conferido.
    """
    from .classificador import candidatos

    def endereco(indice: int) -> int:
        return blocos[indice].indice_fonte if blocos[indice].indice_fonte >= 0 else indice

    detalhes: list[str] = []
    for indice, bloco in enumerate(blocos):
        if bloco.observacao:
            detalhes.append(f"bloco {endereco(indice)} ({bloco.rotulo}): {bloco.observacao}")
        elif bloco.procedencia == "fallback":
            detalhes.append(f"bloco {endereco(indice)}: caiu no bloco genérico")

    pendentes = [i for i in candidatos(blocos) if not blocos[i].observacao]
    sem_rotulo = [
        f"bloco {endereco(i)}: \u201c{_sem_marca(blocos[i].dados.get('titulo', '')).strip() or 'sem título'}\u201d "
        f"ficou como seção de texto comum"
        for i in pendentes
    ]

    if resultado_classificador is None:
        if not pendentes and not detalhes:
            return Achado("Classificação", OK, "todo trecho veio tipado do próprio markdown")
        return Achado(
            "Classificação",
            AVISO,
            f"{len(pendentes)} trecho(s) sem diretiva não passaram por classificação",
            sem_rotulo + detalhes,
        )
    if not resultado_classificador.rodou:
        return Achado(
            "Classificação",
            AVISO,
            "o classificador não rodou",
            [resultado_classificador.motivo_de_nao_rodar] + sem_rotulo + detalhes,
        )
    if detalhes or sem_rotulo:
        casos = sem_rotulo + detalhes
        return Achado("Classificação", AVISO, f"{len(casos)} caso(s) para conferir", casos)
